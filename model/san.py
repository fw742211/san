...
from model.linear_an import Adaptation_Network, Discrimator
...
class PISDistBlock(nn.Module):
    ...
    def __init__(self, shapelet, shapelet_info=None, len_ts=5, alpha=-10,
        ...
        if self.start_position < 0:
            self.left_padding = - self.start_position
            self.start_position = 0
        self.end_position = int(shapelet_info[2] + window_size)
        self.right_padding = 0
        if self.end_position >= len_ts:
            self.right_padding = self.end_position - len_ts
            self.end_position = len_ts
        ...
        self.linear1 = nn.Linear(window_size * 2 + 1, window_size  + 1)
        self.linear2 = nn.Linear(window_size + 1, int(window_size/2) + 1)
        self.linear3 = nn.Linear(int(window_size/2) + 1, 1)
        self.linear = nn.Linear(window_size * 2 + 1, 1)
    ...
    def forward(self, x, ep):
        ...
        if ep == 0 and self.training:
            min_value, _ = torch.min(dist1.detach(), 2)
            max_value = torch.max(min_value.detach())
            if max_value > self.max_norm_dist:
                self.max_norm_dist.data = max_value
        dist1 = 1 - dist1/self.max_norm_dist
        dist1 = F.pad(dist1, (self.left_padding, self.right_padding), "constant", 0)

        out = self.linear1(dist1)
        out = self.linear2(out)
        out = self.linear3(out)
        # out = self.linear(dist1)
        return out
...
class SAN(nn.Module):
    def __init__(self, shapelets_info, shapelets , len_ts, num_classes, sge=0, window_size=20, bounding_norm=100, alpha=0.2):
        ...
        self.linear_latent = nn.Linear(int(len_ts * 0.25), num_classes)

        self.adaptation_network = Adaptation_Network(len_ts=len_ts)
        self.discrimator = Discrimator(len_ts=len_ts)
        self.dropout = nn.Dropout(0.0)
        self.alpha = alpha

    def forward(self, x, ep):
        # Transform Net
        transformed_x, latent_space = self.adaptation_network(x)

        # PPSN
        y = self.pshapelet_layer(transformed_x, ep)
        y = torch.relu(y)

        out_x = self.discrimator(x)
        out_transformed_x = self.discrimator(transformed_x)

        if ep > self.sge:
            y = self.alpha*self.linear_latent(latent_space) + self.linear3(y)
        else:
            y = self.linear_latent(latent_space)

        y = self.dropout(y)
        y = torch.squeeze(y, 1)
        return y, out_x, out_transformed_x
