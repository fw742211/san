...
def binary_cross_entropy(pred, target):
    true_dist = torch.full_like(pred, fill_value=target).to(pred.device)

    return F.binary_cross_entropy(F.sigmoid(pred), true_dist)
...
def main(args):
    ...
    model = SAN(shapelets_info=shapelets_info, shapelets=shapelets,
    ...
    a_optimizer = torch.optim.AdamW(model.adaptation_network.parameters(),lr=args.lr, weight_decay=args.weight_decay)
    log = Log(log_each=3)
    for epoch in range(args.epochs+1):
        ...
        for b in range(len(training_set_loader)):
            ...
            predictions, x_dis, transformed_x_dis = model(inputs,epoch)
            c_loss = basic_smooth_crossentropy(predictions, targets, smoothing=args.smoothing)
            g_loss = binary_cross_entropy(transformed_x_dis, 1.0)

            loss = c_loss + args.alpha * g_loss
            ...
            predictions, x_dis, transformed_x_dis = model(inputs, epoch)
            d_loss = binary_cross_entropy(x_dis, 1.0)
            d_transformed_loss = binary_cross_entropy(transformed_x_dis, 0.0)
            c_loss = basic_smooth_crossentropy(predictions, targets, smoothing=args.smoothing)

            loss = c_loss + args.alpha * (d_loss + d_transformed_loss)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--sge", default=20, type=int, help="stop-gradient epochs")
    ...
    parser.add_argument("--alpha", default=0.5, type=float, help="Fixed at 0")

    parser.add_argument("--batch_size", default=16, type=int, help="Batch size used in the training and validation loop.")
    parser.add_argument("--epochs", default=1500, type=int, help="Total number of epochs.")
